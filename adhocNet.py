...
class Mininet(object):
    ...
    def addHost(self, name, cls=None, **params):
        ...
        if not cls:
            cls = self.host
        print "net.py---------------------addHost() method---------------------------------------custom cls before"
        h = cls(name, **defaults)
        print "net.py---------------------addHost() method---------------------------------------custom cls after"
        h.type = 'host'
        self.hosts.append(h)
    ...
    def addStation(self, name, cls=None, **params):
        ...
        if not cls:
            cls = self.station
        print "net.py-------------------------addStation() method-------------------------call cls() method-----------before"

        sta = cls(name, **defaults)
        print "net.py-------------------------addStation() method-------------------------call cls() method-----------after"
    ...
    def configHosts(self):
        ...
        for host in self.hosts:
            # info( host.name + ' ' )
            print "net.py -------------mininet()--------configHosts()----------------------------------------------------", host
            intf = host.defaultIntf()
            if intf:
                print "net.py-------------------------------mininet()-----------------------------------------configHosts()", intf
                host.configDefault()
                print "net.py----------------------configHosts()----------------------------------------------configDefault() ----- after"
            else:
    ...
    def buildFromTopo(self, topo=None):
        ...
        # Xian: modify for loop to implement wifi
        i = 0
        for hostName in topo.hosts():
            if 'sta' in str(hostName):
                print "adHocNet.py ---------mininet()--------------buildFromTopo()------call addStation() before"
                positionParameters = ['10,20,0', '15,25,10', '20,30,20', '40,30,40']
                self.addStation(hostName, self.host, position=positionParameters[i], **topo.nodeInfo(hostName))
                print "adHocNet.py ---------mininet()--------------buildFromTopo()------call addStation() after"
            else:
        ...
        print "adHocNet.py----------------------mininet() cla---------------configureWifiNodes()---------------wifiwifi before"
        self.configureWifiNodes()
        print "adHocNet.py----------------------mininet() cla---------------configureWifiNodes()---------------wifiwifi after"
        info('\n*** Adding link(s):\n')
        for station in self.stations:
            self.addHoc(station, ssid='adhocNet', mode='g')
        #xian: add lines for draw graph and node move
        self.plotGraph(max_x=100, max_y=100)
        self.propagationModel('friisPropagationLossModel', sL=2)
#        self.seed(random.randint(0, 100))
#        self.startMobility(startTime=0, model='RandomDirection', max_x=100, max_y=100, min_v=0.5, max_v=0.8)
#        i = 1
#        for station in self.stations:
#            if i ==1 :
#                self.mobility(station, 'start', time=i, position='40,30,0')
#                self.mobility(station, 'stop', time=i+1, position='10,20,0')
#            i = i+1
#        self.stopMobility(time=i)
        print "darw graphy     darw graphy darw graphy darw graphy darw graphy darw graphy darw graphy darw graphy darw graphy "
    ...
    def build(self):
        ...
        self.configHosts()
        print "net.py-----------------------------------build()-------------------configHosts() after"
        if self.xterms:
            self.startTerms()
    ...
    def mobility(self, *args, **kwargs):
        ...
        print "adhocNet.py+++++++++++++++++++++++++++++++++mobility() method++++++++++++++++++call mininetwifi.configureMobility()"
