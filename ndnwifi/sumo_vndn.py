...
#    else:
    ...
    for hostName in vndnTopo.hosts():
        if 'car' in str(hostName):
            sumo_vndn.addCar(hostName, wlans=1, **vndnTopo.nodeInfo(hostName))
        else:
            sumo_vndn.addHost(hostName, **vndnTopo.nodeInfo(hostName))
        x=x+1
        info(hostName + ' ')
...
#            info(switchName + ' ')
    ...
    sumo_vndn.propagationModel("logDistancePropagationLossModel", exp=2.5)

    print "*** Setting bgscan"
    sumo_vndn.setBgscan(signal=-45, s_inverval=5, l_interval=10)

    print "*** Configuring Propagation Model"
    sumo_vndn.propagationModel("logDistancePropagationLossModel", exp=2)
    ...
    info('\n*** Adding link(s):\n')
    i=0
    while i<len(sumo_vndn.accessPoints)-1:
        sumo_vndn.addLink(sumo_vndn.accessPoints[i], sumo_vndn.accessPoints[i+1])
        i=i+1
    ...
    for rsu in sumo_vndn.accessPoints:
