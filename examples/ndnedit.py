...
class MiniEdit( Frame ):
    ...
    def exportScript( self ):
        ...
        if len(fileName) > 0:
            ...
            f.write(b"\n")
            if not isWiFi:
                f.write(b"from minindn.minindn import Minindn\n")
            args = ''
            if hasController:
            ...
            if isWiFi:
                f.write(b"from minindn.wifi.minindnwifi import MinindnWifi\n")
                args = b''
                if hasStation:
                    args += b' Station'
                if hasAP:
            ...
            f.write(b"from minindn.apps.app_manager import AppManager\n")
            f.write(b"from minindn.apps.nfd import Nfd\n")
            ...
            f.write(b"\n")
            if not isWiFi:
                f.write(b"    ndn = Minindn(noTopo=True,\n")
            else:
                f.write(b"    ndn = MinindnWifi(noTopo=True,\n")
            ...
            f.write(b"\n")
            if isWiFi:
                f.write(b"    nodes = net.hosts + net.stations\n")
            else:
                f.write(b"    nodes = net.hosts\n")
            f.write(b"    ndn.initParams(nodes)\n")
            f.write(b"    nfds = AppManager(ndn, nodes, Nfd)\n")
    ...
    def about( self ):
        ...
        if about is None:
            ...
            about.title( 'About' )
            desc = self.appName + ': a simple network editor for Mini-NDN'
            version = 'NDNEdit ' + MINIEDIT_VERSION
            author = 'Originally by: Bob Lantz <rlantz@cs>, April 2010'
            enhancements = 'Enhancements by: Ramon Fontes and the Mini-NDN team, Since May 2021'
            www = 'http://github.com/named-data'
    ...
    def build( self ):
        ...
        return (minindn, isWiFi)


    def postStartSetup( self, isWiFi ):
        ...
        nodes = None
        if isWiFi:
            nodes = self.net.hosts + self.net.stations
        else:
            nodes = self.net.hosts
        self.minindn.initParams(nodes)
        nfds = AppManager(self.minindn, nodes, Nfd, logLevel="DEBUG")
    ...
    def start( self ):
        "Start network."
        if self.net is None:
            self.minindn, isWiFi = self.build()
